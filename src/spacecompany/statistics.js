...
Game.statistics = (function () {
	...
	instance.initialise = function () {
		this.createStatistic(
			"manualResources",
			Game.constants.statisticCategoryGeneral,
			"Resources Mined By Hand"
		);
		...
		for (var i = 1; i <= Game.constants.maxTier; i++) {
			this.createStatistic(
				"tierOwned" + i,
				Game.constants.statisticCategoryGeneral,
				"Tier " + i + " Machines Owned"
			);
		}

		this.createStatistic(
			"tabsUnlocked",
			Game.constants.statisticCategoryUnlockable,
			"Tabs Unlocked",
			7
		);
		this.createStatistic(
		...
		this.createStatistic(
			"sessionTime",
			Game.constants.statisticCategoryTiming,
			"Session time",
			0,
			STATISTIC_TYPE.TIME
		);
	...
	instance.updateUnlockedTabs = function () {
		...
		tabCount +=
			$.inArray("solCenterTopTab", resourcesUnlocked) >= 0 ? 1 : 0;
	...
	instance.save = function (data) {
		...
		for (var id in this.entries) {
			if (this.entries[id].value > 0) {
				data.statistics.entries[id] = {
					v: this.entries[id].value,
					va: this.entries[id].valueAlltime,
				};
			}
		}
	};
	...
	instance.load = function (data) {
		...
		if (data.statistics) {
			if (
				data.statistics.version &&
				data.statistics.version === this.dataVersion
			) {
				for (var id in data.statistics.entries) {
					if (this.entries[id]) {
						this.setValue(
							id,
							data.statistics.entries[id].v,
							data.statistics.entries[id].va
						);
					}
				}
			}
	...
	instance.loadLegacy = function (data) {
		if (data.handMined) {
			this.setValue("manualResources", data.handMined, data.handMined);
		}
		if (data.tier1) {
			this.setValue("tierOwned1", data.tier1, data.tier1);
		}
		if (data.tier2) {
			this.setValue("tierOwned2", data.tier2, data.tier2);
		}
		if (data.tier3) {
			this.setValue("tierOwned3", data.tier3, data.tier3);
		}
		if (data.tier4) {
			this.setValue("tierOwned4", data.tier4, data.tier4);
		}
		if (data.tier5) {
			this.setValue("tierOwned5", data.tier5, data.tier5);
		}
		if (data.tier6) {
			this.setValue("tierOwned6", data.tier6, data.tier6);
		}
		if (data.tabsUnlockedNum) {
			this.setValue(
				"tabsUnlocked",
				data.tabsUnlockedNum,
				data.tabsUnlockedNum
			);
		}
		if (data.resourcesUnlockedNum) {
			this.setValue(
				"resourcesUnlocked",
				data.resourcesUnlockedNum,
				data.resourcesUnlockedNum
			);
		}
		if (data.techsResearchedNum) {
			this.setValue(
				"techResearched",
				data.techsResearchedNum,
				data.techsResearchedNum
			);
		}
		if (data.placesExploredNum) {
			this.setValue(
				"placesExplored",
				data.placesExploredNum,
				data.placesExploredNum
			);
		}
		if (data.wondersBuiltNum) {
			this.setValue(
				"wondersBuilt",
				data.wondersBuiltNum,
				data.wondersBuiltNum
			);
		}
		if (data.wondersActivatedNum) {
			this.setValue(
				"wondersActivated",
				data.wondersActivatedNum,
				data.wondersActivatedNum
			);
		}
		if (data.secondsTotal) {
			this.setValue("timePlayed", data.secondsTotal, data.secondsTotal);
		}
	};
